...
export class TagList {
    ...
    templates : {[key: string]: Template | null} = {}
     ...
     * @returns {buffer}
     */
    _generateListMessageRequest(instanceID = 0, program?: string | null) {

        const { LOGICAL, DATA } = CIP.EPATH.segments;

        let pathArray : Buffer[] = [];
     ...
     */
    _parseAttributeListResponse(data : Buffer, program?: string | null) {
        ...
        while (pointer < data.length) {
            ...
            const lastTag = this.tags.findIndex(tag => {
                return (tag.instance_id === `${instanceID}` && tag.program === program); 
            });
    ...
    _getTagTypeNames () {
        for (const tag of this.tags) {
            ...
            let template = this.templates[tag.type.code];
            if(!tag.type.typeName && template) {
                tag.type.typeName = template._name;
            }
        }
    }
     ...
     * @returns {Promise}
     */
    getControllerTags(PLC, program : string | null = null) {
        ...
        return new Promise<void>(async (resolve, reject) => {
            for (const tag of this.tags) {
                if (tag.type.structure && !this.templates[tag.type.code]) {
                    ...
                    let template : Template | null = new Template();
            ...
            while(foundTemplate) {
                ...
                for (const temp in this.templates) {
                    let members = this.templates[temp]?._members || []
