...
export class TCPController extends EventEmitter {
    ...
    configInstance: {assembly?: any, size?: number};
    outputInstance: {assembly?: any, size?: number};
    inputInstance: {assembly?: any, size?: number};
    tryingToConnect: boolean;
    workers: { read: Queue<any>; write: Queue<any>; group: Queue<any>; };
    // static rpi: undefined;
    // static timeout_sp: undefined;
    // static state: undefined;

    static connect(address, arg1, configInstance, outputInstance, inputInstance) {
    ...
    public  async connect(IP_ADDR: string, SLOT = 0, configInstance, outputInstance, inputInstance){
        ...
        const sessid = await this.ethernet.connect(IP_ADDR, this.timeout_sp);   
     ...
     */
    async disconnect() {
        if (this.ethernet.established_conn === true) {
    ...
    async forwardOpen() {
        ...
        const paramsOT = CIP.ConnectionManager.build_connectionParameters(owner.Exclusive, connectionType.PointToPoint, priority.Scheduled, fixedVar.Fixed, 6 + (this.outputInstance.size || 0));
        const paramsTO = CIP.ConnectionManager.build_connectionParameters(owner.Exclusive, connectionType.PointToPoint, priority.Scheduled, fixedVar.Fixed, 2 + (this.inputInstance.size || 0));
        ...
        this.ethernet.establishing_conn = true;
        this.ethernet.established_conn = false;

        this.ethernet.write_cip(forwardOpenPacket); // We need to bypass unconnected send for now

        const readPropsErr = new Error("TIMEOUT occurred while trying forwardOpen Request.");
        
        // Wait for Response
        const data = await promiseTimeout<Buffer>(
        ...
        this.ethernet.id_conn = OTconnID;
        this.ethernet.established_conn = true;
        this.ethernet.establishing_conn = false;
    ...
    async forwardClose() {
        ...
        this.ethernet.write_cip(forwardClosePacket); // We need to bypass unconnected send for now
     ...
     * @memberof ENIP
     */
    write_cip(data, connected?: boolean, timeout = 10, cb = null) {
        const { UnconnectedSend } = CIP;
        let msg;
      
        const isConnected = this.ethernet.established_conn;
        if (isConnected === false) {
            msg = UnconnectedSend.build(data, this.state.controller.path);
        } else {
            msg = data;
        }
        this.ethernet.write_cip(msg, isConnected, timeout, cb);
    }
    ...
    _handleSendRRDataReceived(srrd) {
        ...
        switch (service - 0x80) {
            ...
            case MULTIPLE_SERVICE_PACKET: {
                ...
                let services : number = data?.readUInt16LE(0) || 0;
                let offsets : number[] = [];
                let responses : any[] = [];

                // Build Array of Buffer Offsets
                for (let i = 0; i < services; i++) {
                    offsets.push((data?.readUInt16LE(i * 2 + 2) || 0));
                }
                ...
                const length = (data?.length || 0) - offsets[offsets.length - 1];
    ...
    _handleSendUnitDataReceived(sud) {
        ...
        switch (service - 0x80) {
            ...
            case MULTIPLE_SERVICE_PACKET: {
                ...
                // Get Number of Services to be Enclosed
                let services = data?.readUInt16LE(0) || 0;
                let offsets : number[] = [];
                let responses : any[] = [];
