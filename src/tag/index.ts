...
export class Tag extends EventEmitter {
    ...
    // controller_value: boolean;

    constructor(tagname: string, program?: string | null, datatype? : number | null, keepAlive = 0, arrayDims = 0, arraySize = 0x01) {
        super();
     ...
     */
    set type(type: string | number | null | undefined) {
    ...
    set controller_value(newValue) {
        if (!equals(newValue,this.state.tag.controllerValue)) {
            let lastValue = this.state.tag.controllerValue;
     ...
     * @memberof Tag
     */
    generateReadMessageRequest(value: any = null, size : number | null = null) {
        ...
        switch (this.state.tag.type) {
            case SINT:
                this.controller_value =
                    (data.readInt8(2) & (1 << (tag.bitIndex || 0))) == 0 ? false : true;
                break;
            case INT:
                this.controller_value =
                    (data.readInt16LE(2) & (1 << (tag.bitIndex || 0))) == 0 ? false : true;
                break;
            case DINT:
            case BIT_STRING:
                this.controller_value =
                    (data.readInt32LE(2) & (1 << (tag.bitIndex || 0))) == 0 ? false : true;
                break;
            default:
                throw new Error(
     ...
     * @memberof Tag
     */
    generateWriteMessageRequest(value : any = null, size = 0x01) {
    ...
    generateWriteMessageRequestForBitIndex(value) {
        ...
        switch (tag.type) {
            case SINT:
                ...
                buf.writeUInt8(value ? 1 << (tag.bitIndex || 0) : 0, 2); // or mask
                buf.writeUInt8(value ? 255 : 255 & ~(1 << (tag.bitIndex || 0)), 3); // and mask
                break;
            case INT:
                buf = Buffer.alloc(6);
                buf.writeInt16LE(2); //mask length
                buf.writeUInt16LE(value ? 1 << (tag.bitIndex || 0) : 0, 2); // or mask
                buf.writeUInt16LE(value ? 65535 : 65535 & ~(1 << (tag.bitIndex || 0)), 4); // and mask
                break;
            case DINT:
            case BIT_STRING:
                buf = Buffer.alloc(10);
                buf.writeInt16LE(4); //mask length
                buf.writeInt32LE(value ? 1 << (tag.bitIndex || 0) : 0, 2); // or mask
                buf.writeInt32LE(value ? -1 : -1 & ~(1 << (tag.bitIndex || 0)), 6); // and mask
                break;
            default:
                throw new Error(
    ...
    generateWriteMessageRequestForAtomic(value, size) {
        ...
        if(!tag.type) throw new Error("Tag type not found");
        ...
        switch (tag.type) {
            case SINT:
                if (Array.isArray(value)) {
                    ...
                    if(typeof(tag.value) !== 'number') break;
            ...
            case LINT:
                ...
                if (Array.isArray(value)) {
                    ...
                    if(typeof(tag.value) !== 'bigint') break;
