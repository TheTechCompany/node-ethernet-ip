...
export class Controller extends EthernetIP {
     ...
     * @memberof ENIP
     */
     public async connect (IP_ADDR, SLOT = 0){
    ...
    async readWallClock() {
        ...
        const date = new Date(wallClockArray.toString());
    ...
    async getControllerTagList(tagList: TagList, program = null) {
     ...
     * @memberof Controller
     */
    async _readTag(tag: Tag, size = null) {
    ...
    get tagList() {
        const tags = this.state.tagList?.tags.filter(tag => (!tag.type.reserved));

        return tags?.map((tag) => {
            const structureObj = tag.type.structure ? this.state.tagList?.getTemplateByTag(tag.name) : null;

            const structure = structureObj?._members?.reduce((prev, curr) => {
                return {
                    ...prev,
                    [curr.name || '']: curr.type.typeName
                }
            }, {})
    ...
    newTag(tagname, program: string | null = null, subscribe = true, dataType: Types = Types.BOOL, keepAlive: number = 0, arrayDims = 0, arraySize = 0x01) {
        ...
        if (template) {
            //REMIND: struct 4rd param used to be null
            tag = new Structure(tagname, this.state.tagList, program, dataType, keepAlive, arrayDims, arraySize)
            if (subscribe)
                this.subscribe(tag)
            return tag
        } else {

            //REMIND: tag 3rd param used to be null
          let tag = new Tag(tagname, program, dataType, keepAlive, arrayDims, arraySize)
            if (subscribe)
                this.subscribe(tag)
            return tag
        }
    ...
    getTagByName(name) {
        return Object.values(this.state.subs?.state.tags).find(({state}: any) => state.tag.name === name) 
    }
}
